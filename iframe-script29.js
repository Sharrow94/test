function createModal() {
    ...
    const addListnerForAcceptBtn = setInterval(function () {
        try {
            const iframeDocument = iframe.contentDocument || iframe.contentWindow.document;
            console.log("addListnerForAcceptBtn: "+iframeDocument);
            if (iframeDocument) {
                iframeDocument.addEventListener('click', (event) => {
                    const iframeDocument = iframe.contentDocument || iframe.contentWindow.document;
                    const acceptDialog = iframeDocument.getElementById('630__dialogBox');
                    console.log("listener")
                    console.log(acceptDialog,event.target,event.target.classList.contains('globalPrimaryButton'))
                    console.log(event.target.id)
                    if (acceptDialog && event.target && event.target.classList.contains('globalPrimaryButton')) {
                        console.log('3.14')
                        modal.style.display = 'none';
                        iframeContainer.innerHTML = '';
                    }
