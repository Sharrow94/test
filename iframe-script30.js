function createModal() {
    ...
    const addListnerForAcceptBtn = setInterval(function () {
        try {
            const acceptDialog = iframeDocument.getElementById('630__dialogBox');
            console.log("addListnerForAcceptBtn: "+acceptDialog);
            if (acceptDialog) {
                acceptDialog.addEventListener('click', (event) => {
                    console.log("listener")
                    console.log(event.target,event.target.classList.contains('globalPrimaryButton'))
                    console.log(event.target.id)
                    if (event.target && event.target.classList.contains('globalPrimaryButton')) {
                        console.log('3.14')
                        modal.style.display = 'none';
                        iframeContainer.innerHTML = '';
                    }
